function validar() {
    ...
    var senha_digitada = form_senha.senha.value;
    var old = form_senha.old_pass.value;
    ...
    var senhaHash = calcMD5(senha_digitada);
...
function validar_email() {
  var email_novo = form_info.email.value;
  var emails = form_info.lista_emails.value;
  var email_ref = form_info.email_ref.value;
  var email = "";

  if (email_novo != email_ref){
...
function func_atual(){
    var atual_form = form_xp_add.atual.value;
    if (atual_form){
        if (atual_form == 1){
...
function func_atual_edit(count){
    var atual = `atualedit${count}`;
    var atual_edit = document.getElementById(atual).value;
    var fim = `fim_e${count}`;

    if(atual_edit){
        if (atual_edit == 1){
            document.getElementById(fim).required = false;
